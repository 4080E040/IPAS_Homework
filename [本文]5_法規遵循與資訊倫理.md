...
ISO/IEC 29100:2011 is applicable[適用的] to natural[自然] persons and organizations[組織] involved[涉及] in 

specifying[指定], procuring[採購], architecting[建築設計], designing[設計], developing[發展], testing[測試], maintaining[維護], administering[管理], and operating[操作] 

information[消息] and communication technology systems or services 

where privacy[隱私] controls[控制] are required[需要] for the processing[處理中] of PII.
```
```
...
A computer security[安全] audit[審計] is a manual[手冊] or systematic[系統的] measurable[可測] technical[技術] assessment[評定] of a system[系統] or application[應用]. 

Manual[手動] assessments[評定] include[包括] interviewing[面試] staff[員工], performing[履行] security vulnerability[脆弱性] scans[掃描], 
reviewing[回顧] application and operating[操作] system access[訪問] controls[控制], and analyzing[分析] physical[物理] access[訪問] to the systems. 

Automated[自動] assessments, or CAAT's, include system generated[發生的] audit reports[報告] or using software[軟體] to monitor[監控] 
and report changes[變化] to files and settings on a system. 
Systems can include personal[個人] computers, servers, mainframes[大型機], network routers[路由器], switches[交換器].
```
```
稽核類型 security audit
第一方稽查(內部稽查)
